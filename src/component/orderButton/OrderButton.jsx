...
const OrderButton = ({item}) =>{
	...
	const addToCart = (id,name,price) =>{
		console.log(id)
		dispatch({type:ADD_TO_CART,payload:{id:id,name:name,price:price}})
	}
	const removeFromCart = (id,name,price) =>{
		dispatch({type:REMOVE_FROM_CART,payload:{id:id,name:name,price:price} })
	}

	return (
		selectedItems.hasOwnProperty(item._id.toString()) 
		?
			<Button.Group floated='right' className="mx-5" color="teal" >
				<Button  icon='add' onClick={()=>addToCart(item._id,item.name,item.price)}/>
